...
def fit_Remapped_RV(N, points,flag = False):
	...
	for j in range(0,len(slice.slices)):
		cyl_coord_pts.append(cart2cylinder(slice.slices[j][:,0],slice.slices[j][:,1],bins[j]*np.ones(len(slice.slices[j][:,2]))))
	...
	for i in range(0,len(cyl_coord_pts)):
		for j in range(0,len(cyl_coord_pts[i][0])):
			layers.append([cyl_coord_pts[:,0][i][j],cyl_coord_pts[:,1][i][j],cyl_coord_pts[:,2][i][j]])
	...
	remapped_NURBS_RV_surf = fitting.interpolate_surface(p_ctrlpts, size_u, size_v, degree_u, degree_v)
